  ...
  for(Int_t i = 0; i < fReaderGammas->GetEntriesFast(); i++){ // SFS todo: use faster way to do this
    ...
    if (lIsFromSelectedHeader){
      ...
      if (fDoMaterialBudgetWeightingOfGammasForTrueMesons && fiPhotonCut->GetMaterialBudgetWeightsInitialized()) {
        weightMatBudgetGamma = fiPhotonCut->GetMaterialBudgetCorrectingWeightForTrueGamma(iCandidate, fInputEvent->GetMagneticField());
      }

      Double_t lTotalWeight = (fDoCentralityFlat > 0) ? fWeightJetJetMC*weightMatBudgetGamma*fWeightCentrality[fiCut]
                                                      : fWeightJetJetMC*weightMatBudgetGamma;

      fHistoConvGammaPt[fiCut]->Fill(lPt, lTotalWeight);

      if (fDoPhotonQA > 0 && fIsMC < 2){
        fHistoConvGammaPsiPairPt[fiCut]->Fill(iCandidate->GetPsiPair(),lPt, lTotalWeight);
        fHistoConvGammaR[fiCut]->Fill(iCandidate->GetConversionRadius(), lTotalWeight);
        fHistoConvGammaEta[fiCut]->Fill(iCandidate->Eta(), lTotalWeight);
        fHistoConvGammaPhi[fiCut]->Fill(iCandidate->Phi(), lTotalWeight);

        if ((fDoPhotonQA == 4)||(fDoPhotonQA == 5)){
          fHistoConvGammaInvMass[fiCut]->Fill(iCandidate->GetMass(), lTotalWeight);
          fHistoConvGammaInvMassReco[fiCut]->Fill(GetOriginalInvMass(iCandidate,fInputEvent), lTotalWeight);
        }
      }
      ...
      if ((fDoPhotonQA == 2)||(fDoPhotonQA == 5)){
        ...
        {
          fPtGamma = lPt;
          fDCAzPhoton = iCandidate->GetDCAzToPrimVtx();
          fRConvPhoton = iCandidate->GetConversionRadius();
          fEtaPhoton = iCandidate->GetPhotonEta();
          iCatPhoton = iCandidate->GetPhotonQuality();
  ...
  if(fiPhotonCut->UseElecSharingCut()){
    for(Int_t i = 0;i<GammaCandidatesStepOne->GetEntries();i++){
     ...
     if(fMCEvent && signalRejection && signalRejection!=2){
      ...
      // idea: use set here to check there are two PhotonCandidates that have a track in common
...
void AliAnalysisTaskGammaConvV1::ProcessTruePhotonCandidates(AliAODConversionPhoton *TruePhotonCandidate)
{
  ...
  fHistoTrueGammaPsiPairDeltaPhi[fiCut]->Fill(deltaPhi,TruePhotonCandidate->GetPsiPair(),fWeightJetJetMC*weightMatBudgetGamma);
  if (fiEventCut->IsConversionPrimaryESD( fMCEvent, posDaughter->GetMother(0), mcProdVtxX, mcProdVtxY, mcProdVtxZ)){
    ...
    if (fiEventCut->IsConversionPrimaryESD( fMCEvent, i, mcProdVtxX, mcProdVtxY, mcProdVtxZ)){
      ...
      if(fDoMesonAnalysis){
        ...
        if(fiMesonCut->MesonIsSelectedMC(particle,fMCEvent,fiEventCut->GetEtaShift())){
          ...
          Bool_t kDaughter0IsPrim = fiEventCut->IsConversionPrimaryESD( fMCEvent, particle->GetFirstDaughter(), mcProdVtxX, mcProdVtxY, mcProdVtxZ);
